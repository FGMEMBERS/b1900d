...
setlistener("/instrumentation/flightdirector/lnav", func {
    ...
    if(lnav == 2){AP_hdg.setValue("nav1-hold");BC_btn.setBoolValue(0);if(vnav ==7 ){vnav = 0;}
    			if(getprop("/instrumentation/nav/slaved-to-gps")){
...
update_nav = func {
    ...
    if(slaved == 0){
        ...
        nav_mag_brg = getprop("/instrumentation/nav/heading-deg");
			...
			if (GPS_on.getBoolValue()){
			tmp_brg = getprop("/instrumentation/gps/wp/wp[1]/bearing-true-deg");
			if(tmp_brg == nil){tmp_brg = 0;}
			setprop("/autopilot/settings/true-heading-deg",tmp_brg);
...
####    Copy route manager entries to GPS    ####

setlistener("/autopilot/route-manager/route/num", func {
	var num = cmdarg().getValue();
	if(!GPS_on.getBoolValue()){return;}
	setprop("/instrumentation/gps/wp/wp[1]/waypoint-type","nav");
	var storewpt = getprop("/instrumentation/gps/wp/wp[0]/ID");
	if(num == 0 or num == nil){setprop("/instrumentation/gps/wp/wp[1]/ID","");
		setprop("/instrumentation/gps/wp/wp[1]/altitude-ft",0);
		}else{	
	if(storewpt!= nil){setprop("/instrumentation/gps/wp/wp[0]/ID",getprop("/instrumentation/gps/wp/wp[0]/ID"));}
	setprop("/instrumentation/gps/wp/wp[1]/ID",getprop("/autopilot/route-manager/route/wp/id"));
	setprop("/instrumentation/gps/wp/wp[1]/altitude-ft",getprop("/autopilot/route-manager/route/wp/altitude-ft"));
	}
});
